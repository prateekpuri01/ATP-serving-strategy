...
The data files contain information on ATP matches from 1991-2019, and thus my analysis is restricted to this subset. Further, matches with players with fewer than 50 career ATP matches were cut from the dataset in order to further restrict my analysis to players with significant ATP experience. Lastly, matches in which each player hit fewer than 20 first serves or fewer than 10 second serves were excluded from the dataset to effectively remove matches decided by player retirement or other unusual circumstances. 
...
Strategies (1) and (2) are identical except for the second serve that is hit. Therefore, to assess the differences in probability (Delta P) of winning a second serve point under the two strategies, we can define the following quantity, labeled the 'enhancement metric' (EM):
...
Thus, the EM factor is equivalent to the increase in probability of winning a second serve point associated with switching from strategy (1) to strategy (2). Therefore, an EM factor>0 may imply that strategy (2) would be advantageous over strategy (1) for a particular player, while an EM factor<0 would imply the opposite. 
...
Even though Ivanisevic's career EM is quite high (>.02), we see that his EM factor against different opponents varies substantially. Even though, on average, he would appear to benefit from strategy (2), against certain opponents strategy (1) is clearly preferable. 

Certain opponents simply return first/second serves better than other opponents, and these effects will guide which strategy is optimal when playing them. Therefore, we can reconstruct the above tables while restricting ourselves to a player's serving statistics against a particular player. Here, we will restrict ourselves to players who have played each other a minimum of **10 times** so that we may have a reasonable amount of data to sample.
...
After calibrating the winning percentages for both safe and risky strategies for each matchup involving active top-30 players (P1 vs. P2), the following two subplots were obtained. In the first subplot, the MC predicted winning percentages (WP) for P1 against P2 when P1 uses both a safe and risky strategy are plotted against one another. Here, the dahsed line represents the dividing line that determines when a risky strategy is beneficial. Points below this line represent matchups that would benefit from a safe strategy and are colored red, while points above this line represent matchups that would benefit from a risky strategy and are colored blue. Further the size of the points represents the change in winning percentage (absolute value) that would be expected from switching from a safe to a risky strategy. The large red points represent matchups where a risky approach would significantly hurt a player, whereas the large blue points represent matchups where such an approach would significantly help the player (for scale, the max/min of the change in winning percentage distribution are (+11%)/(-23%), respectively).  

![](/data_visualizations/MC_results_visualization.png?raw=true)

As can be seen from the first plot, P1 could only expect to benefit from a risky approach in roughly 20% of matchups. 

Further, as seen in the second subplot, 80% of the matchups in which risky strategies are preferred fall within the set of matches where the safe strategy WP <0.4. On the other hand, just 10% of players who have a safe strategy WP >0.6 would benefit from a strategy switch. In other words, if you're consistently losing against a player, a change in strategy may be helpful, but if you're already winning often then changing tactics is unlikely to help you additionally.  
