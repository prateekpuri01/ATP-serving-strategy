...
For each matchup involving active top-30 players (P1 vs. P2), the following plot displays the MC predicted winning percentage (WP) for P1 against P2 when P1 is using both a safe and risky serving strategy. Here, the dahsed line represents the dividing line between when a risky strategy is beneficial. Points below this line represent matchups that would benefit from a safe strategy and are colored red, while points above this line represent matchups that would benefit from a risky strategy and are colored blue. Further the size of the points represents the change in winning percentage (absolute value) that would be expected from switching to a risky strategy. The large red points represent matchups where a risky approach would significantly hurt a player, whereas the large blue points represent matchups where such an approach would significantly help the player (for scale, the max/min of the change in winning percentage distribution are (+11%)/(-23%), respectively).  
